...
| Teleport eggs (v3.9.0, `CONFIG.teleportEgg`) animate the slot's **transform/transition** (`.tele-out`/`.tele-in`) and put the shimmer/arrival `animation` on the SVG child | Same rule: never set `animation` on `.egg-slot` itself. `teleportEgg()` in smash.js also writes `egg._pos` so a re-render keeps the new spot |
