...
## `/polished` — alternate visual skin (web only, v3.1.0)
`polished.html` is **generated by `build.js`** from `index.html` (same markup, same bundle) with
`polished.css` injected after `bundle.min.css` and `polished.js` after `bundle.min.js`. Vercel's
`cleanUrls` serves it at `/polished`. Same origin ⇒ same save.
| Rule | Why |
|------|-----|
| Never edit `polished.html` by hand; edit `polished.css` / `polished.js` and run `node build.js` | It is a build output; hand edits are overwritten and drift from `index.html` |
| The skin must stay override-only: nothing in `index.html`, the four source stylesheets, `render.js` or the bundle may change for it | The whole point is that it is revertable by deleting `polished.{css,js}` + `buildPolished()` |
| `polished.js` swaps `makeEggSVG` at runtime and calls `renderEggTray()` once on load; it must keep the SVG size/viewBox and never add `.egg-slot.<state>` rules with `animation` in `polished.css` | Layout, hit areas and the smash-animation cascade invariants must hold on both routes |
| Not in `sitemap.xml`, not in `sw.js` `STATIC_ASSETS`, `noindex` | It is a preview surface for the same game, not a second landing page |
Design system (four rules): dark hue-shifted outline outside + accent as an inner line; banded light (warm top-left, teal rim bottom-right, no gradients/blur/circles); one teal-navy field with kraft/cork as the warm counterpoint (quotes and notes live on kraft tags); the egg is the hero — per-egg ramps in `polished.js` are tuned against the tray field `#1e3a55`.
