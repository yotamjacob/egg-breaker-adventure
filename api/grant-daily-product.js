// Vercel serverless proxy for grant-daily-product.
...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(`${SUPABASE_URL}/functions/v1/grant-daily-product`, {
      method: 'POST',
      headers: {
    ...
    return res.status(500).json({ error: err.message || 'Grant request failed' });
  }
}
